...
- ${useCase === 'normal' ? 'Generate a well-defined and practical strategy in the `Thought` section, summarizing your next move and its objective.' : 'Compose a step-by-step approach in the `Thought` part, specifying your next action and its focus.'}
 - Use wait() ONLY when there is an explicit loading indicator (spinner/skeleton/progress in address bar). Avoid back-to-back wait(). If no visible change after one step, prefer: refresh via hotkey(key='f5') or navigate(content='...') (browser), retype the intended input, or navigate_back().
...
- You should NOT use google when you need to search for information, use baidu.com instead.
 - Use wait() ONLY when there is an explicit loading indicator (spinner/skeleton/progress in address bar). Do NOT use wait() twice in a row. If no visible change after one step, prefer: hotkey(key='f5')/navigate(content='...') (browser), retype input, or navigate_back().
