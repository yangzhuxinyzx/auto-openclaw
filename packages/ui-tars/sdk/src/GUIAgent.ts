  ...
  async run(
    ...
    try {
      ...
      while (true) {
        ...
        const tShotEnd = Date.now();
        logger.info('[Perf][Agent] screenshotMs=', tShotEnd - tShotStart);
        ...
        const tInvokeEnd = Date.now();
        logger.info('[Perf][Agent] invokeTotalMs=', tInvokeEnd - tInvokeStart);
        ...
        for (const parsedPrediction of parsedPredictions) {
          ...
          if (!signal?.aborted && !this.isStopped) {
            ...
            logger.info('[Perf][Agent] executeMs=', tExecEnd - tExecStart, parsedPrediction.action_type);
