...
export class UITarsModel extends Model {
  ...
  async invoke(params: InvokeParams): Promise<InvokeOutput> {
    ...
    logger?.info(`[Perf][Model] imagePreprocessMs=${tPreEnd - tPreStart}`);
    ...
    logger?.info(`[Perf][Model] buildMessagesMs=${tMsgEnd - tMsgStart}`);
    ...
    try {
      ...
      logger?.info(`[Perf][Model] parseActionMs=${tParseEnd - tParseStart}`);
      return {
        prediction,
        parsedPredictions,
        costTime,
